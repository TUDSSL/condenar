...
{
    unsigned int size:24;    // short size
    unsigned int tag:4;
...
void Z_ClearZone(memzone_t *zone)
{
    ...
    block->size = (zone->size - sizeof(memzone_t)) >> 2;
}
...
void Z_Init(void)
{
    ...
    block->size = (mainzone->size - sizeof(memzone_t)) >> 2;
}

static inline memblock_t* getMemblockPrev(memblock_t *mb)
{
    return (memblock_t*) (mb->prev);
}
...
void* Z_Malloc2(int size, int tag, void **user, const char *sz)
{
    ...
    } while (base->tag != PU_FREE || (base->size << 2) < size);

    // found a block big enough
    extra = (base->size << 2) - size;

    if (extra > MINFRAGMENT)
    {
        // there will be a free fragment after the allocated block
        newblock = (memblock_t*) ((byte*) base + size);
        newblock->size = extra >> 2;

        newblock->tag = PU_FREE;
        newblock->user = 0;
        newblock->prev = base;
        newblock->next = base->next;
        getMemblockNext(newblock)->prev = newblock;

        base->next = newblock;
        base->size = size >> 2;
    }
    free_memory -= base->size << 2;
    // no need to write else base->size = base->size...
    //
    ...
    printf("Mall: occ. %d lrgst %d Addr %04x. BlkSz %d %s\r\n", -free_memory, -largest_occupied, base, (base->size << 2), sz);
#endif
...
void (Z_Free)(void *p)
{
    ...
    if (block->user && block->tag != PU_FREE) // Nullify user if one exists
    {
        *(getMemblockUser(block)) = NULL;
    }
    // free memory
    free_memory += (block->size << 2);
    ...
    if (other->tag == PU_FREE)
    {
        ...
        getMemblockNext(other)->prev = other;
    ...
    if (other->tag == PU_FREE)
    {
        ...
        getMemblockNext(block)->prev = block;

        if (other == mainzone->rover)
            mainzone->rover = block;
    }
#ifdef ZMALLOC_STAT
    printf("Free: occ. %d lrgst %d Addr %04x. BlkSz %d\r\n", -free_memory, -largest_occupied, block, (block->size << 2));
#endif
}
...
void* (Z_Realloc)(void *ptr, size_t n, int tag, void **user)
{
    ...
    if (ptr)
    {
        ...
        memcpy(p, ptr, n <= (block->size << 2) ? n : (block->size << 2));
