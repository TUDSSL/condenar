...
const fixed_t projectiony = (((int64_t)(CHEIGHT * (SCREENWIDTH / 2) * 320) / 200) * FRACUNIT / SCREENWIDTH);
...
#define BETTER_ASM_CODE 0
 #if BETTER_ASM_CODE
 while (l--)
 {
   uint32_t fracShift = 0;
   uint32_t dummyTmp = 0;
   __asm__ volatile 
   (
      ".align(2)\n\t"
...
{    
    ...
    memcpy(columnData,patch + co,cnt);
}
...
FASTFUN static inline short* openingSsptrToAddr(void* ssptr)
{
   return ssptr;
}
FASTFUN static inline void* openingAddrToSsptr(short *ptr)
{
    return ptr;
}
//
...
#define GET_SIZE_FROM_TEXTURE_DATA 1
        ...
        memcpy(columnData,patch  +  8 + 4 * texture->width + xc * (texture->height  + 5) + 3,texture->height);
#else
        ...
        for (; rw_x < rw_stopx; rw_x++)
        {
            ...
            // draw the wall tiers
            {
                ...
                if (toptexture)
                {
                    ...
                    if (mid >= yl)
                    {             
                        if(toptex==NULL) __BKPT(1); //ALECAMAR PORT FIX!              
...
FASTFUN static void R_StoreWallRange(const int start, const int stop)
{
    ...
    if (!backsector)
    {
        ...
        ds_p->sprtopclip_ssptr = (short *) screenheightarray;
        ds_p->sprbottomclip_ssptr =(short *) negonearray;
