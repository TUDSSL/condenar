...
{
    if(pageAddress<(uint32_t*)SRAM_XIP_BASE_ADDRESS){
        LOG_I("flashErasePage address not in XIP range??????");
        SpinLoop();
    }
    memset(pageAddress,0xFF,4096);
}
void programFlashWord(uint32_t *address, uint32_t word)
{
     if(address<(uint32_t*)SRAM_XIP_BASE_ADDRESS){
        LOG_I("programFlashWord address not in XIP range??????");
        SpinLoop();
    }

    if (*address == 0xFFFFFFFF) //I guess check whether it was previously erased?
    {
        *address=word;


        /*Flash_DisableXIP();
        Flash_Write(address,&word,sizeof(word));
        Flash_EnableXIP();*/
...
void* writeLumpToFlashRegion(int lumpnum, uint8_t flashRegion, boolean updateSize)
{
    ...
    //LOG_I("[LUMP] Remaining space: %d %d",currentImmutableFlashOffset,getUserFlashRegionRemainingSpace());
...
void* storeFlashImmutableValues()
{
    ...
    for (int i = 0; i < sizeof(*p_wad_immutable_flash_data) / sizeof(uint32_t); i++)
    {
