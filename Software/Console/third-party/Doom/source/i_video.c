...
{
    _g->screens[0].data = displayData.displayFrameBuffer;
...
static void I_UploadNewPalette(int pal)
{
    ...
    if (_g->gamma)
    {
        ...
        for (int i = 0; i < 256; i++)
        {
            uint16_t r = gammatable[_g->gamma][p_wad_immutable_flash_data->palette_lump[pal * 256 * 3 + 3 * i]];
            uint16_t g = gammatable[_g->gamma][p_wad_immutable_flash_data->palette_lump[pal * 256 * 3 + 3 * i + 1]];
            uint16_t b = gammatable[_g->gamma][p_wad_immutable_flash_data->palette_lump[pal * 256 * 3 + 3 * i + 2]];
            k_color toWrite = K_COLOR_RGB(r,g,b);
            palette[i] = toWrite;            
        }
    }
    else
    {   // save some cycles for those playing with 0 gamma.
           for (int i = 0; i < 256; i++)
        {
            uint16_t r = p_wad_immutable_flash_data->palette_lump[pal * 256 * 3 + 3 * i];
            uint16_t g = p_wad_immutable_flash_data->palette_lump[pal * 256 * 3 + 3 * i + 1];
            uint16_t b = p_wad_immutable_flash_data->palette_lump[pal * 256 * 3 + 3 * i + 2];
            k_color toWrite = K_COLOR_RGB(r,g,b);
            palette[i] = toWrite;     
        }
    }
