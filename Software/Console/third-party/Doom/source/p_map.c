...
msecnode_t* P_AddSecnode(sector_t *s, mobj_t *thing, msecnode_t *nextnode)
{
    ...
    while (node)
    {
        ...
        {
            node->m_thing_sptr = thing; // Yes. Setting m_thing says 'keep it'.
            return (nextnode);
        }
    ...
    node->m_sector_sptr = s;       // sector
    node->m_thing_sptr = thing;     // mobj
    node->m_tprev_sptr = 0L;    // prev node on Thing thread
    node->m_tnext_sptr = nextnode;  // next node on Thing thread
    if (nextnode)
        nextnode->m_tprev_sptr = node; // set back link on Thing
    ...
    if (s->touching_thinglist_sptr)
        getMsecnodeSNext(node)->m_sprev_sptr = node;
//  s->touching_thinglist = node;
    s->touching_thinglist_sptr = node;
    return (node);
}
...
msecnode_t* P_DelSecnode(msecnode_t *node)
{
    ...
    if (node)
    {
        ...
        sn = node->m_snext_sptr;
        if (sp)
            sp->m_snext_sptr = sn;
        else
        {
//      node->m_sector->touching_thinglist = sn;
            getMsecnodeSector(node)->touching_thinglist_sptr = sn;
        }
        if (sn)
            sn->m_sprev_sptr = sp;
