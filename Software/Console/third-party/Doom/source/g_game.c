...
#define SAVEGAME_OFFSET 0
#define SETTINGS_OFFSET 4096

uint8_t* getSavegamePointer(){
    return k_GetStorageReadOnlyAddressForApp(k_getCurrentApp(), NULL) + SAVEGAME_OFFSET;
}

uint8_t* getSettingsPointer(){
    uint8_t* ptr=k_GetStorageReadOnlyAddressForApp(k_getCurrentApp(),NULL);
    if(ptr==NULL) return NULL;
    return ptr + SETTINGS_OFFSET;
}
...
void G_DoLoadLevel()
{
    ...
    P_SetupLevel(false,_g->gameepisode, _g->gamemap, 0, _g->gameskill);
...
{
    doom_save_data_t *saveslots = (doom_save_data_t*) getSavegamePointer();

    for (int i = 0; i < 8; i++)
    {
        if (saveslots==NULL || saveslots[i].save_present != 1)
        {
...
static void G_DoSaveGame(boolean menu)
{
    ...
    byte *savebuffer = (byte*) displayData.displayFrameBuffer;

    {
        uint8_t* saveGameREADONLYptr = getSavegamePointer();
        if(saveGameREADONLYptr==NULL) return;
        
        // read old data
        memcpy(savebuffer,saveGameREADONLYptr,savebuffersize);
    }
    ...
    //
    k_WriteToAppStorage(k_getCurrentApp(), SAVEGAME_OFFSET, savebuffer,savebuffersize);
...
void G_SaveSettings()
{
    ...
    k_WriteToAppStorage(k_getCurrentApp(), SETTINGS_OFFSET, (uint8_t*)&settings, sizeof(settings));
}

void G_LoadSettings()
{
    doom_save_settings_t *settings = (doom_save_settings_t*) getSettingsPointer();
   
    if (settings != NULL && settings->cookie == settings_cookie)
    {
