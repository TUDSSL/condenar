...
{
    return; //We do not support double buffers inside doom, so lets just skip the melting effect
...
{
    bool isNormalStartup = !k_isRecoveringFromACheckpoint();

    if(isNormalStartup) k_screen_RenderLoadingScreen(20,"Loading basic game data...");

    if(isNormalStartup) IdentifyVersion();

    // init subsystems

    if(isNormalStartup) G_ReloadDefaults();    // killough 3/4/98: set defaults just loaded.
    ...
    R_Init();       // here initImmutableFlashRegion() is called too
    ...
    if(isNormalStartup) k_screen_RenderLoadingScreen(30,"Reading game data...");
