    ...
    while (ticks--)
    {
        for (i = 0; i < (SCREENWIDTH); i++)
        {
            ...
            if (y_lookup[i] < SCREENHEIGHT)
            {
                ...
                s = &displayData.displayFrameBuffer[i] +
                    ((SCREENHEIGHT - dy - 1) * SCREENPITCH);
                d = &displayData.displayFrameBuffer[i] +
                ...
                s = &displayData.displayFrameBuffer[i] +  y_lookup[i] * SCREENPITCH; 
                d = &displayData.displayFrameBuffer[i] + y_lookup[i] * SCREENPITCH;
                for (j = 0 ; j < dy; j++)
                {
                    *d = *s;
                    d += SCREENPITCH;
