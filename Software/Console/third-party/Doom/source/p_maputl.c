...
{                                                      // link into subsector
    ...
    if (!(thing->flags & MF_NOSECTOR ))
    {
        ...
        if (sec->thinglist_sptr)
            getSectorThingList(sec)->sprev_sptr = thing;
        sec->thinglist_sptr = thing;
        ...
        thing->touching_sectorlist_sptr = _g->sector_list; // Attach to Thing's mobj_t
    ...
    if (!(thing->flags & MF_NOBLOCKMAP ))
    {
        ...
        if (blockx >= 0 && blockx < _g->bmapwidth && blocky >= 0 && blocky < _g->bmapheight)
        {
            void**link = &_g->blocklinks_sptrs[blocky * _g->bmapwidth + blockx];
            thing->bprev_sptr = 0;
            thing->bnext_sptr = *link;
            if (*link)
                ((mobj_t*) (*link))->bprev_sptr = thing;
            *link = thing;
        }
        else
        {
...
boolean P_BlockThingsIterator(int x, int y, boolean func(mobj_t*))
{
    mobj_t *mobj;
    if (!(x < 0 || y < 0 || x >= _g->bmapwidth || y >= _g->bmapheight))
        for (mobj = (_g->blocklinks_sptrs[y * _g->bmapwidth + x]);
