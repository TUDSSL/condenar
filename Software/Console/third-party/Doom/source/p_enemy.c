...
static void P_RecursiveSound(sector_t *sec, int soundblocks, mobj_t *soundtarget)
{
    ...
    sec->soundtarget_sptr = soundtarget;
        ...
        for (bx = xl; bx <= xh; bx++)
        {
            for (by = yl; by <= yh; by++)
            {
                ...
                if (!P_BlockThingsIterator(bx, by, PIT_VileCheck))
                {
                    ...
                    actor->target_sptr = _g->corpsehit;
                    A_FaceTarget(actor);
                    //actor->target = temp;
                    actor->target_sptr = temp;
...
void A_VileTarget(mobj_t *actor)
{
    ...
    actor->tracer_sptr = fog;
    //P_SetTarget(&fog->target, actor);
    fog->target_sptr = actor;
    //P_SetTarget(&fog->tracer, actor->target);
    fog->tracer_sptr = actor->target_sptr;
    A_Fire(fog);
}
