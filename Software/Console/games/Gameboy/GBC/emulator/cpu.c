...
void gb_cpu_dump(struct gb *gb) {
     ...
     LOG_I("Flags: %c %c %c %c  IME: %d\n",
             cpu->f_z ? 'Z' : '-',
             cpu->f_n ? 'N' : '-',
             cpu->f_h ? 'H' : '-',
             cpu->f_c ? 'C' : '-',
             cpu->irq_enable);
     LOG_I("PC: 0x%04x [%02x %02x %02x]\n",
             cpu->pc,
     ...
     LOG_I("SP: 0x%04x\n", cpu->sp);
     LOG_I("A : 0x%02x\n",   cpu->a);
     LOG_I("B : 0x%02x  C : 0x%02x  BC : 0x%04x\n",
             cpu->b, cpu->c, gb_cpu_bc(gb));
     LOG_I( "D : 0x%02x  E : 0x%02x  DE : 0x%04x\n",
             cpu->d, cpu->e, gb_cpu_de(gb));
     LOG_I("H : 0x%02x  L : 0x%02x  HL : 0x%04x\n",
...
static void gb_i_undefined(struct gb *gb) {
     ...
     LOG_I("Undefined instruction instruction 0x%02x at 0x%04x\n", instruction, instruction_pc);
     SpinLoop();
}
