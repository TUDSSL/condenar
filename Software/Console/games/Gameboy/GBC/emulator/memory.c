...
void gb_spu_sync(struct gb* gbptr) {
     //Do nothing, used to be sound related
}
...
uint8_t gb_memory_readb(struct gb *gb, uint16_t addr) {
     ...
     //LOG_I("Unsupported read at address 0x%04x\n", addr);
...
void gb_memory_writeb(struct gb *gb, uint16_t addr, uint8_t val) {
     ...
     //LOG_W("Unsupported write at address 0x%04x [val=0x%02x]\n", addr, val);
}
