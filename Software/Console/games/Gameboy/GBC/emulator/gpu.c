...
static void gb_gpu_draw_cur_line(struct gb *gb) {
     ...
     if(IsGBCSkippingThisFrame()) return; //Only render 1/2 of the frames since the GB wants to run at 60FPS
     ...
     if (gb->gbc) {
          gb_sdl_draw_line_gbc(gb, gpu->ly, line);
     } else {
          gb_sdl_draw_line_dmg(gb, gpu->ly, line);
     }
}
     ...
     if (master_enable != gpu->master_enable) {
          ...
          if (master_enable == false) {
               ...
               for (i = 0; i < GB_LCD_HEIGHT; i++) {
                    gb_sdl_draw_line_dmg(gb, i, line);
               }
