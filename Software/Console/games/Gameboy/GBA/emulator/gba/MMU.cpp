...
bool init()
{	
	//Fast clear of the memory, just in case
	memset((uint8_t*)SRAM_XIP_BASE_ADDRESS, 0, 0x40000 + 0x4000 + 0x8000 + 0x400 + 0x20000 + 0x400 + 0x400);


	workRAM     = (uint8_t*)SRAM_XIP_BASE_ADDRESS; //   new u8[0x40000];
	bios        = workRAM + 0x40000; //new u8[0x4000];
	internalRAM = bios + 0x4000; //new u8[0x8000];
	paletteRAM  = internalRAM + 0x8000; //new u8[0x400];
	vram        = paletteRAM + 0x400; //new u8[0x20000];
	oam         = vram + 0x20000; //new u8[0x400];
	ioMem       = oam + 0x400; //new u8[0x400];
	//End of shared memory used: (ioMem + 0x400). Total memory: 445,440B
...
u32 read32(u32 address)
{
	...
	if (address & 3)
	{
		if (settings_log_channel_enabled(LOG_UNALIGNED_MEMORY))
 		{
			am_util_stdio_printf("Unaligned word read: %08x at %08x\n", address, CPU::armMode ?
...
void write32(u32 address, u32 value)
{
	...
	if (address & 3)
	{
		if (settings_log_channel_enabled(LOG_UNALIGNED_MEMORY))
		{
			am_util_stdio_printf("Unaligned word write: %08x to %08x from %08x\n",
			    value,
			    address,
...
void write16(u32 address, u16 value)
{
	...
	if (address & 1)
	{
		if (settings_log_channel_enabled(LOG_UNALIGNED_MEMORY))
		{
			am_util_stdio_printf("Unaligned halfword write: %04x to %08x from %08x\n",
			    value,
			    address,
			    CPU::armMode ? CPU::armNextPC - 4 : CPU::armNextPC - 2);
		}
...
static T unreadable(u32 address)
{
		...
		{
			am_util_stdio_printf("Illegal read: %08x at %08x\n", address, CPU::armMode ?
			    CPU::armNextPC - 4 : CPU::armNextPC - 2);
		}
#endif
...
static T readBios(u32 address)
{
	...
	if (CPU::reg[15].I >> 24)
	{
		...
		if (address < 0x4000)
		{
			...
			//LOG_I("Reading protected bios memory %d %d\n", address, value);
		}
		else
		{
...
static void unwritable(u32 address, T value)
{
	...
	{
		am_util_stdio_printf("Illegal write: %02x to %08x from %08x\n",
			value,
			address,
			CPU::armMode ? CPU::armNextPC - 4 : CPU::armNextPC - 2);
	}
