...
int cpuTotalTicks = 0;
#define LCD_TICK_DIVIDER 1
static int lcdTicks = 1008 * LCD_TICK_DIVIDER;
	...
	if ((DM0CNT_H & 0x8000) && (dmamask & 1))
	{
		if (((DM0CNT_H >> 12) & 3) == reason)
		{
			...
			if (settings_log_channel_enabled(LOG_DMA0))
			{
				...
				if (DM0CNT_H & 0x0400)
					count <<= 1;
				am_util_stdio_printf("DMA0: s=%08x d=%08x c=%04x count=%08x\n", dma0Source, dma0Dest,
				    DM0CNT_H,
				    count);
			}
#endif
	...
	if ((DM1CNT_H & 0x8000) && (dmamask & 2))
	{
		if (((DM1CNT_H >> 12) & 3) == reason)
		{
			...
			if (reason == 3)
			{
				...
				{
					am_util_stdio_printf("DMA1: s=%08x d=%08x c=%04x count=%08x\n", dma1Source, dma1Dest,
					    DM1CNT_H,
					    16);
				}
#endif
	...
	if ((DM2CNT_H & 0x8000) && (dmamask & 4))
	{
		if (((DM2CNT_H >> 12) & 3) == reason)
		{
			...
			if (reason == 3)
			{
				...
				{
					int count = (4) << 2;
					am_util_stdio_printf("DMA2: s=%08x d=%08x c=%04x count=%08x\n", dma2Source, dma2Dest,
					    DM2CNT_H,
					    count);
				}
#endif
	...
	if ((DM3CNT_H & 0x8000) && (dmamask & 8))
	{
		if (((DM3CNT_H >> 12) & 3) == reason)
		{
			...
			if (settings_log_channel_enabled(LOG_DMA3))
			{
				...
				if (DM3CNT_H & 0x0400)
					count <<= 1;
				am_util_stdio_printf("DMA3: s=%08x d=%08x c=%04x count=%08x\n", dma3Source, dma3Dest,
				    DM3CNT_H,
				    count);
			}
#endif
...
void CPUUpdateRegister(u32 address, u16 value)
{
	...
	{ // we need to place the following code in { } because we declare & initialize variables in a case statement
		...
		if (change && !((value & 0x80)))
		{
			if (!(DISPSTAT & 1))
			{
				lcdTicks = 1008 * LCD_TICK_DIVIDER;
				//      VCOUNT = 0;
...
gboolean CPULoadBios(uint8_t *biosPtr)
{
	int size = 0x4000; //16KB

	memcpy(bios, biosPtr, size);
	//bios = biosPtr;
...
void CPULoop(int ticks)
{
	...
	for (;;)
	{
		if (!holdState)
		{
			...
			//am_util_stdio_printf("HALT. PC: %d\n", CPU::armNextPC);
		}
